...
exports.getResponseCount = function(projectId, taskID, option) {
    return new Promise(function(resolve, error) {
        var connection = db.get();
        connection.queryAsync('SELECT COUNT(response) as count, task_id FROM response WHERE project_id=? AND task_id=? AND response=?', [projectId, taskID, option]).then(
            function(data) {
                console.log("get response count " + data[0].count);
                resolve(data);
            }, function (err) {
                error(err);
            }
...
exports.getResponseCountForAllOptions = function(projectId, taskID) {
  return new Promise(function(resolve, error) {
      var connection = db.get();
      connection.queryAsync('SELECT response as option, COUNT(response) as count, task_id FROM response WHERE project_id=? AND task_id=? Group By response', [projectId, taskID]).then(
          function(data) {
              console.log("get response count " + data);
              resolve(data);
          }, function (err) {
              error(err);
          }
...
exports.storeCairnMessage = function(userID, projectID, hitID, message, progress, type, time_shown_to_player, time_player_submitted, task_name) {
    return new Promise(function(resolve, error) {
        ...
        connection.queryAsync('insert into cartoscope_cairns (user_id,project_id,hit_id,message,level_number, cairn_type, time_shown_to_player, time_player_submitted, task_name) VALUES(?,?,?,?,?,?,FROM_UNIXTIME(?),FROM_UNIXTIME(?),?) on DUPLICATE KEY UPDATE message= VALUES(message)',
            [userID, projectID, hitID, message, progress, type, time_shown_to_player, time_player_submitted, task_name]).then(resolve, error);
    });
};
