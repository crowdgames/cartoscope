 ...
 */
const getCairnTypeFromAbbr = (cairnAbbr: string): CairnType => {
  if (!cairnAbbr) return CairnType.none;
...
const insertSidebarMsg = (message: string) => {
...
const pickRandomInArray = (arr: any[]): any => {
  if (!arr) return;
 ...
 */
const modifyGraphCairnBar = (graph: any, ratio: number) => {
  graph.innerHTML = `
...
const serialializeQueryParams = (queryParams: queryParam[]): string => {
  if (!queryParams.length) return "";

  let partiallySerialized = queryParams.map(
    (qp: queryParam) => `${qp.key}=${qp.value}`
  );

  return partiallySerialized.join("&");
};

const convertLongitudeToTileX = (longitude: number, zoom: number) => {
  return Math.floor(((longitude + 180) / 360) * Math.pow(2, zoom));
};

const convertLatitudeToTileY = (latitude: number, zoom: number) => {
  return Math.floor(
    ((1 -
      Math.log(
 ...
 */
const taskImageUrl = (dataset: string, imageName: string) =>
  `/api/tasks/getImage/${dataset}/${imageName}`;
