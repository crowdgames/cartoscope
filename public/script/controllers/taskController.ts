...
interface ITaskScope extends ng.IScope {
  ...
  point_array: string[];
  next_per2: number;
  showSurvButton: boolean;
  cont_button: string;
  showCairnElements: boolean;
  showPlayerSidebar: boolean;
  next_per: number;
  mturkbarStyle: { width: string };
  showDebug: boolean;
  showSoapstoneForm: boolean;
  showSidebar: boolean;
  ...
  showHelpHintModal: () => void;
  showTutorial: (option: string) => void;
  helpHintModalActiveTab: string;
...
taskApp.controller("taskController", [
  ...
  function (
    ...
    if (vm.cairnsInfoArray) {
      vm.tasksUntilNextCairn =
        getRandomIntInclusive(
          parseInt(vm.cairnsInfoArray[1]),
          parseInt(vm.cairnsInfoArray[2])
        ) - 1;
    } else {
    ...
    vm.handleCairns = () => {
      console.assert(
        vm.cairnState === CairnState.noCairn,
        "cairn state is not noCairn, despite the main task showing"
      );

      if (vm.cairnsInfoArray === null || vm.cairnsInfoArray[0] === "n") return;
      ...
      if (vm.tasksUntilNextCairn > 0) {
        ...
        resetCairnCounter();
        if (cairnMode === CairnType.soapstone) vm.startSoapstoneCairn();
        else if (cairnMode === CairnType.emoji) vm.startEmojiCairn();
        else if (cairnMode === CairnType.graph) vm.showGraph = true;
    ...
    vm.flagImage = () => {
      const shouldFlag = window.confirm(
        "Are you sure you want to flag the image?"
      );
      if (shouldFlag) {
        taskService
    ...
    vm.handleGraphCairn = (option) => {
      ...
      if (vm.countyes + vm.countno <= 5) {
        ...
        let ratio = Math.round(
          (vm.countyes / (vm.countyes + vm.countno)) * 100
        );
        if (vm.startgraph == 1) {
          // vm.graphcairnbar.innerText = "";
    ...
    vm.populateMsgSidebar = (numMsgs: number) => {
      ...
      let existingMessages = Array.from(
        ...
        .then((response) => {
          if (response && response.length) {
            response
              ...
              .filter(
                (message: string) =>
                  existingMessages.filter((msg) => msg === message).length === 0
              )
    ...
    vm.startSoapstoneCairn = () => {
      ...
      if (!vm.wasBackHit) vm.populateMsgSidebar(5);
    };
    ...
    vm.soapstoneMsgTypePick = () => {
      ...
      vm.soapstoneTypes.forEach((key: string) => {
    ...
    vm.storeMsgTypePick = () => {
      ...
      vm.selectedMsgType = (
        soapstoneForm.elements[0] as HTMLSelectElement
      ).value;
    };
    ...
    vm.soapstoneMain = () => {
      ...
      vm.replaceFormElemsWithSoapstone(
        soapstoneForm,
        vm.soapstones[vm.selectedMsgType]
      );
      vm.extractSoapstone();
    ...
    vm.extractSoapstone = () => {
      ...
      vm.soapstoneFormValues = Array.from(
        document.getElementById("soapstone-form")!.children
      )
        .map(
          (child) =>
    ...
    vm.attemptExtractInitial = () => {
      ...
      let initial = (textField as HTMLInputElement).value
    ...
    vm.addEmojiToPhysics = (emojiToAdd: string) => {
      ...
      let newEmoji = Bodies.rectangle(
        (Math.random() * window.innerWidth) / 4 + 30,
        50,
        40,
        40,
        {
    ...
    function getBeforeImage() {
      if (!vm.dataset || vm.tasks.length == 0) {
        ...
        return (
          "/api/tasks/getImage/" + vm.dataset + "/before_" + vm.tasks[0].name
        );
      }
    }
      ...
      if (!userData) {
        ...
        if (!parseInt(vm.data.has_survey)) {
          ...
          if (vm.code == heatMapProject2 || vm.code == heatMapProject1) {
            window.location.replace(
              "/survey.html#/survey?code=" + vm.code + "&userType=kiosk"
            );
          } else {
            var hitID =
              $location.search().hitID || $location.search().trialID || "kiosk";
    ...
    function handleFinish() {
      ...
      if (!userData) {
        ...
        if (vm.chaining != -1) {
          ...
          $http
            .get(
              "/api/project/getNextProjectChain/" + encodeURIComponent(workerID)
            )
            .then(
              function (data) {
                ...
                if (next_codes.length == 0) {
                  ...
                  var random_pick =
                    next_codes[Math.floor(Math.random() * next_codes.length)];
      ...
      $http.get("/api/project/getTutorial/" + vm.code).then(function (tdata) {
        ...
        tutData.forEach((item: any) => {
          ...
          if (item.answer == option && item.ask_user) {
            ...
            if (
              vm.data.template.selectedTaskType == "tagging" ||
    ...
    $scope.showHelpHintModal = function () {
      ...
      $scope.triggerClick("#showTutModalButton");
      const option =
        $scope.model.data.template.options.length > 0 &&
          ...
          if (
            ...
            item.icon =
              $scope.point_array_filtered[
                $scope.point_array_filtered.length - 1
              ];
            item.setIcon(
              $scope.point_array_filtered[
                $scope.point_array_filtered.length - 1
    ...
    function addMarker(m_lat, m_lon) {
      NgMap.getMap({ id: "main_map" }).then(function (point_map) {
        if (
          typeof $scope.pointMarkers !== "undefined" &&
        ...
        .then((response) => {
            ...
            $http
              ...
              .then(function (data) {
                ...
                $http.get(queryline).then(function (vote_data) {
                  ...
                  //Map initialization
                  NgMap.getMap({ id: "flight_path" }).then(function (
                    flight_map
                  ) {
          ...
          if (vm.showMarkerPoints || vm.viewMarkerPoints) {
            ...
            NgMap.getMap({ id: "main_map" }).then(function (point_map) {
              ...
              if (vm.showMarkerPoints) {
                d3.csv(
                  "/images/files/" + vm.point_csv_file,
                  function (csv_data) {
