...
taskApp.controller("taskController", [
    "$scope",
    "$location",
    "$http",
    "userData",
    ...
    function ($scope, $location, $http, userData, $window, $timeout, NgMap, $q, $sce, heatMapProject1, heatMapProject2) {
        ...
        $scope.geoMarkers = [];
        $scope.icon_array = defaultIconArray();
        ...
        $scope.point_array = defaultPointArray();
        //for chaining, look for the parameter:
        vm.chaining = $location.search().chain; //Redundant call
        //Disable chaining
        vm.chaining = -1;
        //progress bars
        $scope.next_per2 = 0;
        ...
        vm.hitID =
            $location.search().hitID || $location.search().trialID || "kiosk";
        let hitIDSplit = vm.hitID.split("_");
        vm.cairnsInfoArray =
            hitIDSplit.length > 2 && hitIDSplit[2].split("-")[0] === "cairns"
                ? hitIDSplit[2].split("-").slice(1)
                : null;
        ...
        if (vm.cairnsInfoArray) {
            vm.tasksUntilNextCairn =
                getRandomIntInclusive(parseInt(vm.cairnsInfoArray[1]), parseInt(vm.cairnsInfoArray[2])) - 1;
        }
        else {
            vm.tasksUntilNextCairn = 5;
        }
        vm.nextCairnToShow = CairnType.none;
        vm.showGraph = false;
        vm.handleCairns = () => {
            console.assert(vm.cairnState === CairnState.noCairn, "cairn state is not noCairn, despite the main task showing");
            if (vm.cairnsInfoArray === null || vm.cairnsInfoArray[0] === "n")
                return;
            let cairnMode = vm.cairnsInfoArray
                ? getCairnTypeFromAbbr(vm.cairnsInfoArray[0])
                : CairnType.none;
            ...
            if (vm.tasksUntilNextCairn === -1) {
                resetCairnCounter();
                vm.tasksUntilNextCairn =
                    vm.tasksUntilNextCairn > 5 ? 5 : vm.tasksUntilNextCairn;
            }
            if (vm.tasksUntilNextCairn > 0) {
                ...
                resetCairnCounter();
                if (cairnMode === CairnType.soapstone)
                    vm.startSoapstoneCairn();
                else if (cairnMode === CairnType.emoji)
                    vm.startEmojiCairn();
                else if (cairnMode === CairnType.graph)
                    vm.showGraph = true;
                else if (cairnMode === CairnType.both) {
                    // If we are supposed to show both cairns, alternate between them, starting with a random one
                    if (vm.nextCairnToShow === CairnType.none)
                        vm.nextCairnToShow = pickRandomInArray([
                            CairnType.soapstone,
                            CairnType.emoji,
                        ]);
                    if (vm.nextCairnToShow === CairnType.soapstone) {
                        vm.nextCairnToShow = CairnType.emoji;
                        vm.startSoapstoneCairn();
                    }
                    else {
                        vm.nextCairnToShow = CairnType.soapstone;
                        vm.startEmojiCairn();
                    }
                }
            }
        ...
        vm.submitCairn = (baseCairnType, message) => {
            ...
            taskService
                .submitCairn(vm.data.id, vm.hitID, encodeURI(message), cairnType, vm.data.progress - 1, vm.timeCairnShownToPlayer, timeCairnSubmitted, vm.previousTaskName)
        ...
        vm.flagImage = () => {
            ...
            if (shouldFlag) {
                taskService
                    .flagImage(vm.data.id, vm.tasks[0])
                    .then(() => vm.submitResponse());
            }
        };
        ...
        vm.fetchResponse = () => {
            ...
            let yesCountResponse = taskService
                .getResponseCount(projectid, taskname, ResponseOptions.Yes)
                .then((data) => data)
                .catch((err) => false);
            let noCountResponse = taskService
                .getResponseCount(projectid, taskname, ResponseOptions.No)
                .then((data) => data)
                .catch((err) => false);
            if (!yesCountResponse || !noCountResponse)
                return;
        ...
        vm.handleGraphCairn = (option) => {
            ...
            if (vm.startgraph == 0) {
                ...
                graphcairn === null || graphcairn === void 0 ? void 0 : graphcairn.insertAdjacentElement("afterend", vm.graphcairnbar);
                vm.startgraph = 1;
            }
            ...
            if (vm.countyes + vm.countno <= 5) {
                ...
                if (vm.startgraph == 1) {
                    ...
                    // }
                    modifyGraphCairnBar(vm.graphcairnbar, ratio);
        ...
        vm.populateMsgSidebar = (numMsgs) => {
            ...
            let existingMessages = Array.from(document.getElementsByClassName("cairn-message")).map((p) => p.innerText);
            const body = {
            ...
            taskService
                ...
                if (response && response.length) {
                    response
                        ...
                        .forEach((message, idx) => setTimeout(() => insertSidebarMsg(message), idx * msBetweenMessages));
                }
                else {
                    console.error("No relevant soapstone messages found.");
                }
            })
                .catch(() => console.error("No relevant soapstone messages found."));
        };
        ...
        vm.startSoapstoneCairn = () => {
            ...
            document.getElementById("cairn-header").innerText =
                "Would you like to leave a message for other cartoscope players?";
        ...
        vm.soapstoneMsgTypePick = () => {
            ...
            document.getElementById("cairn-header").innerText =
                "What kind of message would you like to leave?";
            vm.selectedMsgType = "";
        ...
        vm.storeMsgTypePick = () => {
            ...
            vm.selectedMsgType = soapstoneForm.elements[0].value;
        };
        ...
        vm.soapstoneMain = () => {
            ...
            vm.extractSoapstone();
            document.getElementById("cairn-header").innerHTML =
                "Build up your message! Your message so far: <br><br>" +
                    vm.soapstoneFormValues;
        };
        vm.soapstones = Object.entries(soapstoneTypesToMessagesMap).reduce((acc, [k, v]) => {
            acc[k] = v.map((stringOrArry) => typeof stringOrArry === "string"
                ? stringOrArry
        ...
        vm.soapstoneSign = () => {
            ...
            avatars.forEach((avatar) => {
                ...
                label.onclick = () => {
                    ...
                    document.getElementById("cairn-header").innerText =
                        "Your chosen avatar: " + avatar;
                };
                form.appendChild(label);
            });
            document.getElementById("cairn-header").innerText =
                "Last step! Sign your message, choose from one of the avatars below!";
        };
        vm.soapstoneFormValues = "";
        /**
        ...
        vm.extractSoapstone = () => {
            ...
            vm.soapstoneFormValues = Array.from(document.getElementById("soapstone-form").children)
                .map((child) => child.localName === "select"
                ? child.value
                : child.innerText.trim() // remove &nbsp from both sides
            )
                .join(" ");
        };
        ...
        vm.soapstoneThankYou = () => {
            ...
            document.getElementById("cairn-header").innerText =
                "Thank you for your submission, we've added it to the pile :)";
        ...
        vm.startEmojiCairn = () => {
            ...
            document.getElementById("cairn-header").innerText =
                "Would you like to play a minigame?";
        ...
        vm.startEmojiCreate = () => {
            ...
            document.getElementById("cairn-header").innerHTML =
                "Would you like to take a break? These emojis represent how other players feel. Feel free to play around with them. <br><br>You can add an emoji if you want to. Pick one that describes how you're feeling!<br>";
        ...
        vm.addEmojiToPhysics = (emojiToAdd) => {
        ...
        // module aliases
        let Engine = Matter.Engine, Render = Matter.Render, Runner = Matter.Runner, World = Matter.World, MouseConstraint = Matter.MouseConstraint, Mouse = Matter.Mouse, Bodies = Matter.Bodies;
        ...
        vm.initializePhysics = () => {
            ...
            var mouse = Mouse.create(vm.render.canvas), mouseConstraint = MouseConstraint.create(vm.engine, {
                mouse: mouse,
                constraint: {
                    stiffness: 0.2,
                    render: {
            ...
            var leftWall = Bodies.rectangle(-45, physHeight, 100, physHeight * 4, {
                isStatic: true,
            });
            var rightWall = Bodies.rectangle(physWidth + 45, physHeight, 100, physHeight * 4, { isStatic: true });
            var topWall = Bodies.rectangle(300, -45, physWidth * 4, 100, {
                isStatic: true,
            });
        ...
        vm.fillPhysicsWithEmojisFromDatabase = () => {
            ...
            taskService.getCairns(body).then((response) => {
                // Fill with emojis from the server
                response.forEach((datum) => vm.addEmojiToPhysics(datum["message"]));
        ...
        vm.getBeforeImageNGS = function () {
            ...
            var g_link = `https://mt1.google.com/vt/lyrs=s&x=${x}&y=${y}&z=${zoom}`;
            return g_link;
        };
        function showModal() {
        ...
        function getTasks() {
            vm.showModal();
            taskService
                .getTask(vm.code, vm === null || vm === void 0 ? void 0 : vm.image_loop)
                .then((response) => {
                var _a;
                vm.dataset = response === null || response === void 0 ? void 0 : response.dataset;
                vm.tasks = response === null || response === void 0 ? void 0 : response.items;
                if (((_a = vm.tasks) === null || _a === void 0 ? void 0 : _a.length) !== 0) {
                    vm.fetchResponse();
                }
                if ((response === null || response === void 0 ? void 0 : response.finishsed) && !vm.image_loop) {
                    vm.finished = true;
                    if (vm.userType == "kiosk") {
                        vm.handleEnd();
                    }
        ...
        function handleFinish() {
            ...
            if (!userData) {
                ...
                if (vm.chaining != -1) {
                    ...
                    $http
                        .get("/api/project/getNextProjectChain/" + encodeURIComponent(workerID))
                        .then(function (data) {
            ...
            if (!userData) {
                ...
                if (!parseInt(vm.data.has_survey)) {
                    ...
                    if (vm.chaining != -1) {
                        ...
                        //get the next projects in the chain
                        $http
                            .get("/api/project/getNextProjectChain/" +
                            ...
                            if (next_codes.length == 0) {
                                ...
                                let queryParams = Object.entries(parms).map(([key, value]) => {
                                    if (key === "code") {
                                ...
                                //go to next task
                                window.location.replace("/consentForm.html#/instruction?" +
                ...
                tutData.forEach((item) => {
                    ...
                    if (item.answer == option && item.ask_user) {
                        ...
                        if (vm.data.template.selectedTaskType == "tagging" ||
                            ...
                            (vm.data.template.selectedTaskType == "mapping" &&
                                vm.data.point_selection == false)) {
        ...
        $scope.showHelpHintModal = function () {
            //activate modal
            $scope.triggerClick("#showTutModalButton");
            const option = $scope.model.data.template.options.length > 0 &&
                $scope.model.data.template.options[0].text;
        ...
        vm.submitAnswer = function (option, option_text) {
            ...
            if (!$scope.showMainTask)
                return;
            if (vm.data.template.selectedTaskType === "ngs" &&
                option_text === "No Image") {
                vm.noImageCounter++;
                if (vm.noImageCounter === 4) {
                    showToast("<b>Please keep going even if you aren't finding images</b>.<br> It is helpful for us to know that these facilities have not been photographed.");
                    vm.noImageCounter = 0;
                }
            }
            else
            ...
            if (vm.showMarkerPoints) {
                ...
                $scope.pointMarkers.forEach(function (item) {
                    ...
                    if (item.icon !=
                        $scope.point_array_filtered[$scope.point_array_filtered.length - 1]) {
                ...
                taskService.submit(body).then(() => vm.submitResponse());
            }
        };
        vm.noImageCounter = 0;
        ...
        function addMarker(m_lat, m_lon) {
            NgMap.getMap({ id: "main_map" }).then(function (point_map) {
                if (typeof $scope.pointMarkers !== "undefined" &&
                    $scope.pointMarkers.length > 0) {
                                ...
                                for (var i = 0; i < csv_data.length; i++) {
                                    ...
                                    if (vm.showMarkerPoints) {
                                        google.maps.event.addListener(point_marker, "click", function () {
                                            ...
                                            this.icon =
                                                $scope.point_array_filtered[marker_indx];
        ...
        const saveParamsOnLoad = () => {
            let params = Object.assign({}, $location.search());
            let workerID = params.workerID || params.participantID;
