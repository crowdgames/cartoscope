...
//
// }]);
module.controller('taskController', ['$scope', '$location', '$http', 'userData', '$window', '$timeout', 'NgMap', '$q', '$sce', 'heatMapProject1', 'heatMapProject2', function ($scope, $location, $http, userData, $window, $timeout, NgMap, $q, $sce, heatMapProject1, heatMapProject2) {
        ...
        vm.handleSoapstones = function () {
            if (vm.data.progress % vm.tasksToCompleteTillSoapstoneCreation === 0) {
                // Show the create modal
                var soapstoneForm = document.getElementById("soapstone-form");
                var soapstone = vm.randomSoapstone();
        ...
        vm.showNewSoapstoneMsg = function () {
            var body = { projectID: vm.data.id };
            // TODO this should be a get
            $http.post('api/tasks/getCairns', body).then(function (serverReturn) {
                if (serverReturn.data.length > 0) {
                    var message = serverReturn.data[0].message;
                    Toastify({
        ...
        vm.replaceFormElemsWithSoapstone = function (form, soapstone) {
            // clear form
            form.innerHTML = '';
        ...
        vm.submitSoapstone = function () {
            var soapstoneFormValues = Array.from(document.getElementById("soapstone-form").children)
                .map(function (child) {
            ...
            $http.post('api/tasks/submitSoapstone', body).then(function (data) {
                console.log(data);
            });
            $("#soapstoneCreateModal").modal('hide');
            console.log(soapstoneFormValues);
        };
        vm.submitEmoji = function (n) {
        ...
        $('#physicsModal').on('shown.bs.modal', function () {
            var Engine = Matter.Engine, Render = Matter.Render, World = Matter.World, MouseConstraint = Matter.MouseConstraint, Mouse = Matter.Mouse, Bodies = Matter.Bodies;
            if (!vm.isPhysicsModalCreated) {
                ...
                var physicsHost = document.getElementById("physicsBody");
                // create a renderer
                ...
                var mouse = Mouse.create(render.canvas), mouseConstraint = MouseConstraint.create(vm.engine, {
                    mouse: mouse,
                    constraint: {
                        stiffness: 0.2,
                        render: {
                ...
                // run the renderer
                Render.run(render);
                var emojiArray = ["angry", "concerned", "grinning", "thinking", "tongue_out", "weary"];
                var randomMoji = emojiArray[Math.floor(Math.random() * emojiArray.length)];
                for (var i = 0; i < 50; i++) {
                    randomMoji = emojiArray[Math.floor(Math.random() * emojiArray.length)];
                    var newEmoji_1 = Bodies.circle(400, 200, 20, {
                        render: {
                            sprite: {
                    ...
                    World.add(vm.engine.world, [newEmoji_1]);
                }
            }
            var newEmoji = Bodies.circle(400, 200, 20, {
                render: {
                    sprite: {
        ...
        vm.showPhysicsModal = function () { return $("#physicsModal").modal('show'); };
        vm.showEmojiModal = function () { return $("#emojiModal").modal('show'); };
        vm.submitPhysics = function () {
                ...
                tutData.forEach(function (item) {
                    ...
                    if (item.answer == option) {
                        ...
                        if ((vm.data.template.selectedTaskType == 'tagging') ||
                            ...
                            (vm.data.template.selectedTaskType == 'ngs') || ((vm.data.template.selectedTaskType == 'mapping') && (vm.data.point_selection == false))) {
            ...
            $http.get('/api/project/getTutorial/' + vm.code).then(function (tdata) {
                ...
                tutData.forEach(function (item) {
                    ...
                    if (item.answer == option) {
                        ...
                        if ((vm.data.template.selectedTaskType == 'tagging') || (vm.data.template.selectedTaskType == 'slider') || (vm.data.template.selectedTaskType == 'ngs') || ((vm.data.template.selectedTaskType == 'mapping') && (vm.data.point_selection == false))) {
                            vm.tutorial.push(obj);
                        }
                        else {
                            vm.tutorialMapping.push(obj);
